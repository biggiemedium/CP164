...
class BST:
    ...
    def _remove_aux(self, node, key):
        ...
        if node is None:
            ...
            if node._left is None and node._right is None:
                ...
                node._right, _ = self._remove_aux(node._right, tmp._value)
    ...
    def _eq_aux(self, node, node2):
        if node is None and node2 is None:
            return True
        if node is None or node2 is None:
            return False
        if node._value != node2._value:  # dont forget the underscores !
            return False
            # we must check each side
        return self._eq_aux(node._left, node2._left) and self._eq_aux(node._right, node2._right)
