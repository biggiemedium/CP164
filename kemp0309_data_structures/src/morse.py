...
def fill_letter_bst(bst, values):
    ...
    for i, k in values:
        local = ByLetter(i, k)  # i.letter, i.code
        bst.insert(local)
...
def fill_code_bst(bst, values):
    ...
    for i, k in values:
        local = ByCode(i, k)  # i.letter, i.code
        bst.insert(local)
...
def encode_morse(bst, text):
    ...
    for t in text:
        if t.isalpha():
            letter = ByLetter(t.upper(), None)
            bs = bst.retrieve(letter)
            # if bs: # we do not need to pass this off as an if - retrieve returns a deepcopy
            value += bs.code + " "
        elif t == ' ':
            value += "\n"
...
def decode_morse(bst, code):
    ...
    for c in code.strip().split(" "):  # we have to strip and split or it will not work
        # if c == '.' or c == '-': we dont need this
        if c == '':
            value += " "
        if c == '\n':
            value += " "
