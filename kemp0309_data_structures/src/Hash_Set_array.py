...
class Hash_Set:
    ...
    def insert(self, value):
        ...
        val = False
        if value not in self._table[hash(value) % len(self._table)]:
            self._table[hash(value) % len(self._table)].append(value)
            self._count += 1
            val = True
    ...
    def remove(self, key):
        ...
        #index = 0
        value = None
        index = self._table[hash(key) % len(self._table)]  # len(self._count)
        if self._count > 0:
            # for i in range(len(self._table[iter])): # remove cant use while or for ??????
            if key in index:
                val = key
                index.remove(key)
                #index[i] = index[-1]
    ...
    def debug(self):
        ...
        # Debug method is very sensative to strings so add this to fix it
        print(f"{len(self._table)} Slots")
        for i in range(len(self._table)):
            # I dont understand this formatting
            print(f"----------------------------------------")
            # must make a new line or validator will not work
            print(f"Slot {i}\n")
            for k in self._table[i]:
                print(k)
            # print("----------------------------------------")
