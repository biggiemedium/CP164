...
class BST:
    ...
    def _flip_aux(self, root):
        ...
        if root is not None:
            ...
            # whenever theleft side is none, it goes to the right of the node above the None value
            self._flip_aux( root._left )
            
            # this makes it check every possible section and flip it
            self._flip_aux( root._right )
    ...
    def are_flipped(self, target):
        ...
        -------------------------------------------------------
        """
        flip = self._are_flipped_aux( self._root, target._root )
    ...
    def _are_flipped_aux(self, left, right ):
        ...
        if right is None and left is None:
            ...
            flipped =  ( left._value == right._value and self._are_flipped_aux( left._left, right._right ) and # entire section is a comparison
                         self._are_flipped_aux( left._right, right._left ) )                                # which will be true or false
            
        return flipped # will be either true or false
    ...
    def total_height(self):
        ...
        ---------------------------------------------------------
        """
        height = self._total_height_aux( self._root )
        
        return height
    
    def _total_height_aux(self, root):
        """
        ---------------------------------------------------------
        Returns the total heights of a bst.
        ---------------------------------------------------------
        Parameters:
            node - the current node in the tree (_BST_Node)
        Returns‌​‌​​​​‌​​‌‌‌​‌​​‌​​‌​​‌‌​​​:
        ...
        if root is not None:
            left_dep = self._total_height_aux( root._left )
            right_dep = self._total_height_aux( root._right )
            total_dep = left_dep + right_dep + root._height
    ...
    def furthest(self):
        ...
        if self._root is not None: 
            Ltemp_root = self._root
            Rtemp_root = self._root

            while Ltemp_root._left is not None:
                Ltemp_root = Ltemp_root._left

            while Rtemp_root._right is not None:
                Rtemp_root = Rtemp_root._right

            values.append( Ltemp_root._value )
            values.append( Rtemp_root._value )
